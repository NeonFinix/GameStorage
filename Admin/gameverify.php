...
if(isset($_GET['edit']))
{
    ...
    <form method="POST">
    <h4>Trailer ID</h4>
    <input type="number" name="trailer" id="min_age" value="<?php echo $datag['trailer'] ?>" >
    ...
    if(isset($_POST['Set']))
    {
        ...
        $set = "UPDATE tbl_games set trailer='".$min."' where game_id='".$id."'";
        if($conn->query($set))
        { 
...
<head>
    ...
    <title>Games to Verify</title>
</head>
<body>
<?php
    $get = "select * from tbl_games gs inner join tbl_dev d on gs.company_id=d.company_id inner join tbl_genre gr on gs.genre_id=gr.genre_id where gs.game_status = 0";
    $row = $conn->query($get);
    if($row->num_rows > 0) {
    $i = 1;
    ...
    ?>
                        <div class="bg-secondary rounded h-100 p-4">
                            <h6 class="mb-4">Games to Verify</h6>
                            <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                             <a href="GamesTotal.php?rej=<?php echo $data["game_id"] ?>" >Reject</a>
                                          <a href="GamesTotal.php?sts=<?php echo $data["game_id"] ?>" >Accept</a> 
                                            </td>
                                        </tr>
                                    </tbody>
                            </div>
